...
function accentCss(palette, level, mode, chosenFrame, reading, frame = { scale: 1, opacity: 1, controls: false }) {
    ...
    if (dressControls) {
        ...
        lines.push(`${small} {`, `  border: 1px solid transparent !important;`, `  border-image-source: ${controlArt} !important;`, `  border-image-slice: ${usingArt ? slice : "27%"} !important;`, `  border-image-width: ${Math.max(2, Math.round((usingArt ? 6 : 5) * frame.scale))}px !important;`, `  border-image-outset: 1px !important;`, `  border-image-repeat: ${repeat} !important;`, `}`);
    }
    return lines.join("\n");
}
