...
function accentCss(palette, level, mode, chosenFrame, reading, frame = { scale: 1, opacity: 1, controls: false }) {
    ...
    lines.push(`${both} {`, `  background-color: rgba(${wash}, ${tint}) !important;`, `  background-image: linear-gradient(to ${toDark}, rgba(${lightTint}, ${(0.14 + tint).toFixed(3)}), rgba(${wash}, ${(tint * 0.35).toFixed(3)}), rgba(${wash}, 0.02)) !important;`, `  box-shadow: ${litEdge} rgba(${lightTint}, .32) !important;`, 
    ...
    lines.push(`${root} [${ACCENT_MARK}]:hover { outline-color: rgba(${spark}, .8) !important; border-color: rgba(${spark}, .7) !important; }`, `${root} [${ACCENT_MARK}]:focus-visible { outline-color: rgba(${spark}, .85) !important; }`, `${root} [${ACCENT_MARK}][aria-pressed="true"], ${root} [${ACCENT_MARK}][aria-selected="true"], ${root} [${ACCENT_MARK}][data-on="true"], ${root} [${ACCENT_MARK}][data-active="true"] {`, `  outline-color: rgba(${spark}, .85) !important;`, `  border-color: rgba(${spark}, .8) !important;`, `}`);
    ...
    if (chosenFrame && frame.controls) {
        lines.push(`${small} {`, `  border: 1px solid transparent !important;`, `  border-image-source: ${art} !important;`, `  border-image-slice: ${slice} !important;`, `  border-image-width: ${Math.max(2, Math.round(6 * frame.scale))}px !important;`, `  border-image-outset: 1px !important;`, `  border-image-repeat: stretch !important;`, `}`);
    }
    return lines.join("\n");
}
...
function AiAccentPanel(props) {
    ...
    }, (providers ?? []).map((p) => h("option", { key: String(p.id), value: String(p.id) }, String(p.label)))), h("button", { className: "dshImgSkin-btn", type: "button", disabled: checking, onClick: () => void probeKey() }, checking ? "检查中…" : "检查")), isCustom ? "自定义：下面填 Base URL 与模型 ID" : `默认模型 ${String(current?.model ?? "")}`), keyCheck ? h("p", { className: "dshImgSkin-ok" }, keyCheck) : null, isCustom ? field("base", "Base URL", textInput("accentBaseUrl", { placeholder: "https://your-endpoint/v1" })) : null, field("model", "模型 ID", textInput("accentModel", { placeholder: current?.model ? String(current.model) : "your-model-id" }), isCustom ? undefined : "留空则用该服务商的默认模型"), field("keysrc", "API Key", h("div", { className: "dshImgSkin-inline" }, ...["env", "manual"].map((m) => h("button", {
        key: m,
        type: "button",
        className: "dshImgSkin-btn",
    ...
    }), promptDraft && autoPrompt && promptDraft !== autoPrompt ? "已手改（生成用你改的这份）" : undefined), h("div", { className: "dshImgSkin-inline" }, h("button", { className: "dshImgSkin-btn", type: "button", onClick: () => void preview() }, "取提示词"), h("button", {
        className: "dshImgSkin-btn",
        type: "button",
        "data-variant": "quiet",
        onClick: () => {
    ...
    }, "重置为自动"), h("span", { className: "dshImgSkin-fitlabel" }, `${Number(v.accentCount ?? 2)} 张`), h("input", {
    ...
    return function ImageSkinSection() {
        ...
        sampledColours.length
            ? h("div", { className: "dshImgSkin-swatchRow" }, h("div", { className: "dshImgSkin-inline" }, h("span", { className: "dshImgSkin-fitlabel" }, "取自壁纸"), ...sampledColours.map((c, i) => h("span", { key: `s${i}`, className: "dshImgSkin-swatch", style: { background: `rgb(${c})` }, title: c })), String(v.accentMaterial ?? "") && String(v.accentMaterial) !== "plain"
                ...
                : null), h("span", { className: "dshImgSkin-hint" }, "档位只决定本机画多少；生成的花纹在下面单独应用"))
            : null), 
        // ② 已应用的装饰框：生成物和档位解耦，这里给它自己的开关和三个旋钮。
        h("div", { className: "dshImgSkin-card" }, h("div", { className: "dshImgSkin-cardhead" }, h("div", null, h("span", { className: "dshImgSkin-title" }, "已应用的装饰框"), h("p", { className: "dshImgSkin-sub" }, String(v.accentFrame ?? "")
            ? "来自生图结果，套在面板与弹窗上。粗细/浓淡随时可调。"
            : "还没选生成框。下面生成一张，点它一下就会套上来。")), String(v.accentFrame ?? "")
            ? h("button", { className: "dshImgSkin-btn", type: "button", onClick: () => apply("accentFrame", "") }, "取消应用")
            : null), String(v.accentFrame ?? "")
            ? h("div", { className: "dshImgSkin-inline" }, h("img", { className: "dshImgSkin-framePreview", src: String(v.accentFrame), alt: "" }), h("div", { className: "dshImgSkin-frameKnobs" }, h(SliderRow, {
                key: "fw",
                title: "粗细",
                ...
                onPreview: (n) => reapplyWith?.({ accentFrameScale: n }),
                onCommit: (n) => apply("accentFrameScale", n),
            }), h(SliderRow, {
                key: "fo",
                title: "浓淡",
                hint: "花纹太重会压住面板里的字",
                ...
                onPreview: (n) => reapplyWith?.({ accentFrameOpacity: n }),
                onCommit: (n) => apply("accentFrameOpacity", n),
            }), h("label", { className: "dshImgSkin-switch" }, h("input", {
                type: "checkbox",
                checked: v.accentFrameControls === true,
                onChange: (e) => apply("accentFrameControls", e.target.checked),
            }), "也用在按钮/输入框上")))
            : null, Number(v.accentLevel ?? 0) === 0 && String(v.accentFrame ?? "")
            ? h("p", { className: "dshImgSkin-hint" }, "档位是 0（只取色）——要看到装饰框得把档位调到 1 以上：", h("button", { className: "dshImgSkin-btn", type: "button", "data-variant": "quiet", onClick: () => apply("accentLevel", 1) }, "调到 1"))
