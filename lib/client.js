 ...
 */
function frameDataUri(corner, edge, ornate) {
...
function accentCss(palette, level, mode, chosenFrame) {
    ...
    if (level === 0) {
        lines.push(`body[${BODY_ATTR}][${ACCENT_ATTR}] [${ACCENT_MARK}]{`, `  background-color: rgba(${r}, ${g}, ${b}, ${tint}) !important;`, `}`);
    }
    else {
        ...
        lines.push(`body[${BODY_ATTR}][${ACCENT_ATTR}] [${ACCENT_MARK}]{`, `  background-color: rgba(${r}, ${g}, ${b}, ${tint}) !important;`, `  background-image:`, `    repeating-linear-gradient(45deg, rgba(${r}, ${g}, ${b}, .12) 0 1px, transparent 1px 9px) !important;`, `  background-size: auto !important;`, 
        ...
        `  border: 2px solid transparent !important;`, `  border-image-source: ${frame} !important;`, `  border-image-slice: ${chosen ? "30" : "28"} ${chosen ? "30" : "28"} ${chosen ? "30" : "28"} ${chosen ? "30" : "28"} fill !important;`, `  border-image-width: ${thick}px !important;`, `  border-image-outset: ${level >= 2 ? "5px" : "4px"} !important;`, `  border-image-repeat: stretch !important;`, `}`);
    }
    return lines.join("\n");
}
/** Reflect the configured level + artwork onto the skeleton. */
async function applyAccent(value, mode) {
...
function createSection(scope, modeStore) {
    ...
    return function ImageSkinSection() {
        ...
        const entry = (target, title, sub, locked, lockedHint) => h("button", {
            key: target,
            type: "button",
        ...
        }, h("span", { className: "dshImgSkin-entryTop" }, h("span", { className: "dshImgSkin-entryTitle" }, title), h("span", { className: "dshImgSkin-entryGo" }, locked ? "🔒 需要壁纸" : "进入 →")), h("span", { className: "dshImgSkin-entrySub" }, sub), locked && lockedHint ? h("span", { className: "dshImgSkin-lock" }, lockedHint) : null);
        ...
        const aiPage = h("div", { className: "dshImgSkin-shell" }, back("AI 纹样"), h("p", { className: "dshImgSkin-intro" }, "档位 0 只在本机按壁纸配色给按钮 / 弹窗上色，不联网；档位 1–4 让生图模型画一张装饰边框，生成后在下面挑一张应用。"), h("div", { className: "dshImgSkin-card" }, h(AccentRow, {
            key: "accent",
