 ...
 */
function accentCss(palette, level, mode, chosenFrame, reading) {
...
function AiAccentPanel(props) {
    ...
    }, (providers ?? []).map((p) => h("option", { key: String(p.id), value: String(p.id) }, String(p.label)))), current ? (isCustom ? "自定义：下面填 Base URL 与模型 ID" : `默认模型 ${String(current.model)}`) : "加载中…"), isCustom ? field("base", "Base URL", textInput("accentBaseUrl", { placeholder: "https://your-endpoint/v1" })) : null, 
    ...
    field("model", "模型 ID", textInput("accentModel", { placeholder: current?.model ? String(current.model) : "your-model-id" }), isCustom ? undefined : "留空则用该服务商的默认模型"), field("keysrc", "API Key", h("div", { className: "dshImgSkin-seg" }, ["env", "manual"].map((m) => h("button", {
        key: m,
        type: "button",
...
function createSection(scope, modeStore) {
    ...
    return function ImageSkinSection() {
        ...
        sampledColours.length
            ? h("div", { className: "dshImgSkin-swatchRow" }, h("div", { className: "dshImgSkin-inline" }, h("span", { className: "dshImgSkin-fitlabel" }, "取自壁纸"), ...sampledColours.map((c, i) => h("span", { key: `s${i}`, className: "dshImgSkin-swatch", style: { background: `rgb(${c})` }, title: c }))), h("span", { className: "dshImgSkin-hint" }, "装饰配色由这几种色归纳成一个主色，再按角色分配；偏白的色也留着，做高光和分隔线"))
