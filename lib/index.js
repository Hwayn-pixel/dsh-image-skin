...
function buildPrompt(style, palette, strength, extra, material = "") {
...
async function callProvider(provider, key, body, stage) {
    ...
    if (provider.kind === "dashscope")
        return callDashscope(provider, key, body, stage);
 ...
 */
async function callDashscope(provider, key, body, stage) {
    ...
    const submitOne = async () => {
        ...
        try {
            submitted = await fetch(`${base}/services/aigc/text2image/image-synthesis`, {
                ...
                body: JSON.stringify({
                    ...
                    // n is explicit: 万相 defaults to FOUR pictures when it is omitted, which quietly
                    // quadruples the bill every time "1 张" is asked for. One request = one picture; a
                    // higher count means more requests.
                    parameters: { size: body.size.replace(/x/i, "*"), n: 1 },
                }),
        ...
        if (immediate.length)
            return { ok: true, images: immediate };
        const taskId = payload?.output?.task_id ?? payload?.task_id;
        if (!taskId)
            return { ok: false, error: `响应里没有 task_id：${text.slice(0, 200)}` };
    ...
    for (let i = 0; i < jobs; i++) {
        ...
        if (!one.ok)
            return { ok: false, error: one.error ?? "提交失败" };
        if (one.images?.length)
            images.push(...one.images);
        else if (one.taskId)
            waiting.add(one.taskId);
    }
    if (!waiting.size) {
    ...
    for (let attempt = 0; attempt < 60 && waiting.size; attempt++) {
        ...
        for (const taskId of [...waiting]) {
            ...
            try {
                ...
                return { ok: false, error: `轮询失败：${error.message}` };
            }
            ...
            if (status === "SUCCEEDED") {
                ...
                if (!got.length)
                    return { ok: false, error: `任务成功但没有图片：${pollText.slice(0, 200)}` };
                images.push(...got);
...
function collectDashscopeImages(payload) {
    const out = [];
...
function describeFailure(provider, detail) {
    ...
    try {
        if (provider.kind === "dashscope") {
            ...
            if (res.status === 401 || res.status === 403)
                return { ok: false, detail: "服务返回 401/403：Key 不对、或没开通这项服务" };
        ...
        if (res.status === 401 || res.status === 403)
            return { ok: false, detail: "服务返回 401/403：Key 不对或已过期" };
        if (!res.ok)
            return { ok: true, detail: `服务应答 ${res.status}（这个服务不支持预检，直接生成时才知道）` };
        return { ok: true, detail: "Key 可用" };
    }
    catch (error) {
        return { ok: false, detail: `连不上：${error.message}` };
    }
}
/**
 ...
 */
async function runGeneration(payload, stage) {
    if (!payload?.prompt || typeof payload.prompt !== "string")
        return { status: 400, body: { error: "缺少提示词" } };
    const provider = resolveProvider(payload);
    if (!provider)
        return { status: 400, body: { error: "未知的服务商，或自定义服务商缺少 Base URL / 模型 ID" } };
            ...
            handler: async (req, res) => {
                ...
                try {
                    ...
                    if (req.method === "POST" && url.pathname === `${ROUTE_PREFIX}/gen`) {
                        ...
                        const out = await runGeneration(JSON.parse(body));
                    ...
                    if (req.method === "POST" && url.pathname === `${ROUTE_PREFIX}/gen/stream`) {
                        ...
                        try {
                            const out = await runGeneration(JSON.parse(body), (stageName) => send({ stage: stageName }));
                    ...
                    if (req.method === "POST" && url.pathname === `${ROUTE_PREFIX}/test`) {
                        ...
                        const provider = resolveProvider(payload);
                        if (!provider)
                            return ok(200, { ok: false, detail: "未知的服务商，或自定义服务商缺少 Base URL / 模型 ID" });
                        const { key, from } = resolveKey(payload, provider);
                        if (!key) {
                            return ok(200, {
                                ok: false,
                    ...
                    if (req.method === "POST" && url.pathname === `${ROUTE_PREFIX}/prompt`) {
                        ...
                        const prompt = buildPrompt(String(p.style ?? ""), Array.isArray(p.palette) ? p.palette.slice(0, 6).map(String) : [], Number(p.strength) || 2, String(p.extra ?? ""), String(p.material ?? ""));
