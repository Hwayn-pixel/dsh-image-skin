...
- 上一版让浅色模式**变差了**：阶梯往下走、彩度却极低 → 出来是一片**发灰的浅色**，比原来干净的白还脏。
  根因是数学上的：**HSL 颜色越接近白，彩度就越出不来**（L=0.9 时想看到色相，饱和度得 0.4 以上）。
- 而且亮壁纸会"冲白"半透明的浅色面（白云蓝天盖一层淡蓝还是白）。所以浅色模式改为：
  **面更深、彩度随台阶上升**（层次来自"色变浓"而不是"变灰"），覆盖度也略提高。
- 深色模式不动（已验证好看）。
- **换图即时跟色**：壁纸 URL 一变就**强制重新采样**（不吃缓存——重新上传同名文件也能读到新画面），
  并保留 4s 过渡。浅色模式下原来"看不到更新"，其实就是那层色几乎不可见，与上一条同一个根因。


### 视觉语言：色阶重染 + 命名 token + 液态玻璃

- **白底与"硬"的根因**：DSH 有一批面不读主题 token（硬编码白/深灰）——输入框、按钮、菜单、气泡、标签页，
  之前只接管了大面（bg-base / layer / overlay / sidebar）。
- **改色阶，不追选择器**：DSH 整套色板建在两条静态色阶上（`--dsw-static-neutral-bluish-*` / `neutral-*` /
  `deepseek-*`），语义 token 全部由它们派生。现在**逐级保留明度、只换色相**地重染色阶 → 全站（含设置弹窗、
  标签页、徽章、输入区）一起跟随壁纸，且**对比度关系按构造不变**。
- **命名 token 逐条映射**：输入框落在"面"的下一级（读作凹槽）、菜单/提示高一级、按钮按层级给值、
  悬浮态升一级；主按钮保留"高对比"语义（深色模式浅药丸 / 浅色模式深药丸）**只换色相**，字色不受影响。
- **液态玻璃**：浮动面（dialog / menu / listbox / tooltip / input / textarea / 面板）加
  `backdrop-filter: blur(20px) saturate(1.2)`，配合半透明面与顶部亮边（借光）→ 从"纸片"变"玻璃"。
- 文档：`notes/视觉语言.md`（三条根规则、映射表、可调处）。


### 配色：三条路线 + 色调阶梯（层次感）

- **配色强度**三档，界面上可切：**整屏染色** / **只染三处** / **双声部**。
  - 整屏染色：按壁纸底色重建整套配色，界面与控件都参与；
  - 只染三处：面保持中性，颜色只出现在选中项 / 焦点框 / 一条分隔线；
  - 双声部：面借主色，交互色用画面里的第二个色相。
- **色调阶梯（层次感）**：一个色相、五级台阶（窗口 → 面板 → 控件 → 悬停 → 按下），
  表面只轻声道出色相、强调色才用足饱和度；选中/按下的控件会真正吃下壁纸的颜色，
  并为它单独算一个文字色（对比度校验过）。阶梯的级差随档位变深，档位越高层次越清楚。
- **档位 = 颜色深度**：0 最淡（只描一圈细线）→ 4 最浓并跟着视频壁纸呼吸。
- 取色范围：64px 一帧 → 160px、视频取 3 个时间点、保留 6 色；明亮饱和的颜色不再被压分，
  饱和度取自画面里最鲜艳的可用色；视频换场景（>60°）会重新取种，不再锁死在第一幕。
- 纹饰（AI 生成花纹框）暂时下线，界面入口已移除；代码保留在 git 历史里。
